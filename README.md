...
│   │   ├── geoPathStore.ts            # 地质路径（绘制/编辑/测量/剖面）
...
│   │   │   ├── clipCommon.ts          # 地形裁剪共享类型与工具
│   │   │   ├── useClipDrawing.ts      # 地形裁剪：绘制模式
│   │   │   ├── useClipEditing.ts      # 地形裁剪：顶点编辑
│   │   │   ├── useClipHistory.ts      # 地形裁剪：撤销/重做
│   │   │   ├── useClipPersistence.ts  # 地形裁剪：localStorage 持久化
│   │   │   ├── useKeyboardShortcuts.ts # 键盘快捷键（跨平台 Ctrl/Cmd）
│   │   │   ├── usePathDrawing.ts      # 路径绘制
│   │   │   ├── usePathEditing.ts      # 路径顶点编辑
│   │   │   ├── usePathMeasure.ts      # 路径 geodesic 测距
│   │   │   ├── usePathProfile.ts      # 路径地形剖面采样
│   │   │   ├── usePolygonDrawing.ts   # 多边形绘制与面积/周长测量
│   │   │   ├── usePolygonEditing.ts   # 多边形顶点编辑
...
- **地质路径** — 折线绘制与 geodesic 测距、顶点编辑、地形剖面分析（采样 + SVG 图表）
- **多边形勘测** — 多边形绘制与面积/周长测量、地形坡度分析、逐多边形裁切
