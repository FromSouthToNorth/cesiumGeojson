...
-->
<template>
  <SidePanel :visible="visible" title="地质路径" :disable-escape="store.isDrawing || store.isEditing"
      ...
      <div class="drawing-info">
        ...
        <div class="info-row">
          ...
          <div class="snapping-controls">
            <Switch v-model:checked="store.snappingEnabled" size="small" :checked-children="'开'"
    ...
    <template v-if="!store.isDrawing && store.hasPaths">
      ...
      <ListToolbar :count="store.paths.length" :all-visible="store.paths.length > 0 && store.paths.every((p) => p.show)"
        :search-query="searchQuery" search-placeholder="搜索路径" item-name="路径"
        @toggle-all-visibility="store.toggleAllVisibility()" @remove-all="store.clearAll()"
      ...
      <div class="paths-scroll">
        <div v-for="path in filteredPaths" :key="path.id" class="path-card"
          ...
          <div class="path-header">
            ...
            <div class="path-actions">
              ...
              <Tooltip :title="path.show ? '隐藏' : '显示'">
                <Button type="text" size="small" class="action-btn" :aria-label="path.show ? '隐藏路径' : '显示路径'"
              ...
              <Tooltip title="编辑顶点">
                <Button type="text" size="small" class="action-btn" aria-label="编辑顶点"
          ...
          <div v-if="path.id === store.activePathId" class="path-detail">
            ...
            <ElevationProfile :profile="path.elevationProfile" :loading="store.profileLoading"
            ...
            <div class="detail-section">
              ...
              <template v-else>
                <div class="playback-info-row">
                  <span>已行驶 {{ store.playback.currentDistance.toFixed(1) }} /
                    {{ path.measurements.total.toFixed(1) }} m</span>
                </div>
                <div class="playback-info-row">
                  <span>{{ formatTime(store.playback.currentTime) }} / {{ formatTime(path.measurements.total / 50)
                    }}</span>
                </div>
                <Slider :min="0" :max="1" :step="0.001" :value="store.playback.progress" class="playback-slider"
                ...
                <div class="playback-speed">
                  ...
                  <div class="pb-btn-group">
                    <Button :type="store.playback.speed === 0.5 ? 'primary' : 'default'" size="small"
                      @click="store.setPlaybackSpeed(0.5)">0.5x</Button>
                    <Button :type="store.playback.speed === 1 ? 'primary' : 'default'" size="small"
                      @click="store.setPlaybackSpeed(1)">1x</Button>
                    <Button :type="store.playback.speed === 2 ? 'primary' : 'default'" size="small"
                      @click="store.setPlaybackSpeed(2)">2x</Button>
                    <Button :type="store.playback.speed === 4 ? 'primary' : 'default'" size="small"
    ...
    <EditToolbar v-model:link-edit-enabled="store.linkEditEnabled" :visible="store.isEditing" :can-undo="store.canUndo"
      :can-redo="store.canRedo" :vertex-count="store.positions.length" @finish="store.stopEdit()" @undo="store.undo()"
      @redo="store.redo()" />
  </SidePanel>
